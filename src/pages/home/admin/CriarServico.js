  ...
  const postServicos = async () => {
    if("token" in sessionStorage ? sessionStorage.getItem('token').length > 0 : false){
      try {
        ...
        api.post('/servicos', new URLSearchParams("titulo=" + titulo + "&desc=" + descricao + "&img=" + url_imagem + "&ativo=" + ativo + "&ordem=" + ordem), {headers: {"x-access-token" : sessionStorage.getItem("token")}} );
